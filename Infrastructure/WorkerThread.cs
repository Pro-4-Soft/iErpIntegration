...
namespace Pro4Soft.iErpIntegration.Infrastructure
{
    public class WorkerThread
    {
        ...
        private void Run()
        {
            ScheduleThread.Report($"Thread: [{Name}] started").Wait();
            while (!_pendingStop)
            {
                try
                {
                    ...
                    {
                        worker.LogErrorAsync(new Exception($"Thread: [{Name}]", e)).Wait();
                    }
                }
                catch (Exception e)
                {
                    try { ScheduleThread.ReportErrorAsync(e).Wait(); }
                    catch (Exception ex) { Console.WriteLine(ex); }
                }
            }
            ScheduleThread.Report($"Thread: [{Name}] stopped").Wait();
        }
    }
}
